...
public class HPreferenceFragmentExt extends HtcPreferenceFragment {
	...
	private boolean handleOptionsItemSelected(final Activity act, MenuItem item) {
		if (item.getItemId() == R.id.softreboot) {
			...
			Intent intent = new Intent(act, HAboutScreen.class);
	...
	public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
		...
		if (menuType == 0) {
			...
			OnOffSwitch.setOnCheckedChangeListener(new OnCheckedChangeListener(){
				@Override
				public void onCheckedChanged(HtcToggleButtonLight toggle, boolean state) {
					Helpers.prefs.edit().putBoolean("popup_notify_active", state).commit();
					applyPopupState(state);
				}
			});
			((HActivityEx)getActivity()).actionBarContainer.addRightView(OnOffSwitch);
			...
			menuTest.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					PendingIntent pendingintent = PendingIntent.getActivity(act, 0, new Intent(act, HMainActivity.class), 0x10000000);
					...
					final Notification notification2 = (new Notification.Builder(act))
							...
							.setStyle(new Notification.BigPictureStyle()
								.bigPicture(BitmapFactory.decodeResource(act.getResources(), R.drawable.htc_logo))
			...
			((HActivityEx)getActivity()).actionBarContainer.addRightView(menuTest);
			
			applyPopupState(Helpers.prefs.getBoolean("popup_notify_active", false));
		} else if (menuType == 4) {
			OnOffSwitch = new HtcToggleButtonLight(getActivity());
			OnOffSwitch.setLayoutParams(new android.widget.LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));
			OnOffSwitch.setEnabled(true);
			...
			menuTest.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					final Activity act = getActivity();
					final PendingIntent pendingintent = PendingIntent.getActivity(act, 0, new Intent(act, HMainActivity.class), 0x10000000);
			...
			applyPopupState(Helpers.prefs.getBoolean("better_headsup_active", false));
		} else if (menuType == 5) {
			inflater.inflate(R.menu.menu_sub, menu);
