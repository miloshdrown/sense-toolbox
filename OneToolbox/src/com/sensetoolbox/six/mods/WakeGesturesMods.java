...
public class WakeGesturesMods {
	...
	private static void doWakeUp(Object thisObject, long atTime) {
		XposedBridge.log("[ST] doWakeUp: start at " + String.valueOf(atTime));
		...
		if (mPowerManager != null) {
			XposedBridge.log("[ST] doWakeUp: acquire wakelock");
		...
		XposedBridge.log("[ST] doWakeUp: finished");
	}
	...
	private static void handleGesture(MethodHookParam param) {
		...
		XposedBridge.log("[ST] onHtcGestureMotion: " + String.valueOf(j));
		XMain.pref.reload();
		if (XMain.pref.getBoolean("wake_gestures_active", false)) {
			String prefName = null;
			switch (j) {
